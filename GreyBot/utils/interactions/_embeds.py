...
def embed_code_execution(type: bool,user,output):
    """
            Embedding for code execution
            type - bool : 0 error 1 success
            user - str : name of requestor
    """
    title_letter = [f":exclamation: Error running code",f":white_check_mark:  Code Ran sucessfully"][type]
    embed_color = [discord.Color.red(),discord.Color.green()][type]
    embed = discord.Embed(
        title=title_letter,
        color=embed_color,
        timestamp=datetime.now(),
    ...
    embed.set_footer(text=f"Author: {user}")
    return embed
